...
char * sock_ntop( const  struct  sockaddr *sa, socklen_t salen)
{
     ...
     case  AF_INET: {
         ...
         if  (ntohs( sin ->sin_port) != 0) {
             on_log(portstr,  sizeof (portstr),  ":%d" , ntohs( sin ->sin_port));
             strcat (str, portstr);
         }
         return (str);
     }
/* end sock_ntop */
     ...
     case  AF_INET6: {
         ...
         if  (ntohs(sin6->sin6_port) != 0) {
             on_log(portstr,  sizeof (portstr),  "]:%d" , ntohs(sin6->sin6_port));
             strcat (str, portstr);
             return (str);
         }
         return  (str + 1);
     ...
     case  AF_LINK: {
         ...
         if  (sdl->sdl_nlen > 0)
             on_log(str,  sizeof (str),  "%*s (index %d)" ,
                      sdl->sdl_nlen, &sdl->sdl_data[0], sdl->sdl_index);
         else
             on_log(str,  sizeof (str),  "AF_LINK, index=%d" , sdl->sdl_index);
         return (str);
     }
#endif
     default :
         on_log(str,  sizeof (str),  "sock_ntop: unknown AF_xxx: %d, len %d" ,
                  sa->sa_family, salen);
         return (str);
     }
     return  (NULL);
...
void NetService::sendCmd(CPackage * mCmd)
{
	...
	{
		on_log("send command failed ,no valid socket item or not connect socket tag is %d \n",_tag);
	}
}
